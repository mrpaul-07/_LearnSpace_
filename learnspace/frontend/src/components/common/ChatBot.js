    ...
    <>
      ...
      {open && (
        <div className="fixed bottom-6 right-6 z-40 w-80 sm:w-96 max-w-[calc(100vw-3rem)] h-[32rem] max-h-[calc(100vh-3rem)] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between bg-gradient-to-r from-blue-600 to-blue-700 text-white px-4 py-3 shrink-0">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-white/20 rounded-full flex items-center justify-center text-sm">
                🤖
              </div>
              <div>
                <p className="text-sm font-semibold leading-none">{t('bot.title')}</p>
                <p className="text-xs text-blue-100 mt-0.5">Online</p>
              </div>
            </div>
            <button
